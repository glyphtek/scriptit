...
export function loadEnvironment(
  ...
  for (const filePath of envFilePaths) {
    if (pathExistsSync(filePath)) {
      ...
      if (result.parsed) {
        ...
        logger.debug(
          `loadEnvironment: Loaded ${Object.keys(result.parsed).length} variables from ${filePath}`,
        );
      } else {
  ...
  const interpolatedParams = interpolateEnvVars(
    defaultParams,
  ...
  logger.debug(
    `loadEnvironment: Loaded ${Object.keys(finalEnv).length} total environment variables`,
  );
